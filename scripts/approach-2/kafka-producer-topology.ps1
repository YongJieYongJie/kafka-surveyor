# trace-producer-topology.ps1
#
# Synopsis:
# Herein lies a collection of poorly-written PowerShell scripts for the sole
# purpose of generating the Kafka producer topology of services deployed on
# Kubernetes. In other words, it generates a mapping of each service deployed
# to the Kafka topic(s) that the service produces to.
#
# Prerequisites:
...
function Get-PodOutputKafkaTopics
{
  <#
  .SYNOPSIS
  ...
  if ($?) {
    $topics |
      Select-String -Pattern ",topic=(.+?),type=producer-topic-metrics" |
      foreach { $_.Matches.Groups[1].Value } |
      foreach { Write-Output "$AppLabel|$_"}
  }
}

function Get-KafkaProducerTopology
{
  <#
  .SYNOPSIS
  Generate the producer topology for the Kafka cluster.
  .PARAMETER JmxtermPath
  Path to the Jmxterm jar file (see https://github.com/jiaqi/jmxterm).
  .NOTES
  ...
  E.g.:
  [
    { "source": "driver", "target", "smses-outbound-prod-proto-topic" },
    { "source": "driver", "target", "emails-v2-outbound-prod-proto-topic" },
    { "source": "dwh", "target", "order-pricing-retry-prod-topic" },
    ...
  ]
  #>
  param (
  ...
  # produce to a subset of the topics that the service may produce to (deal to
  ...
  Get-KubeApps | Select-String -Pattern '<none>', 'mysql-exporter-' -NotMatch |
    ...
    foreach {
      $AppLabel = $_.split('|')[0]
      $PodIP = $_.split('|')[1]
      Get-PodOutputKafkaTopics -JmxtermPath $JmxtermPath -PodIP $PodIP -AppLabel $AppLabel
    } |
    foreach { 
      $AppLabel = $_.split('|')[0]
      $TopicName = $_.split('|')[1]
...
Write-Output "[*] Using Jmxterm at ""${env:JMXTERM_PATH}"" (based on environment variable JMXTERM_PATH)"
Get-KafkaProducerTopology -JmxtermPath $env:JMXTERM_PATH
